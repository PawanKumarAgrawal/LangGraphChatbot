# streamlit run VII_streamlit_UI_with_SQLiteDatabaseMemory.py
...
from VI_chatbot_backend_with_SQLiteDatabase import chatbot, retrieve_all_threads, save_label, get_labels
...
def finish_rename(thread_id, key):   # NEW in Lecture 2
    ...
    save_label(thread_id, new_value)   # 🔥 persist in DB
...
if 'thread_labels' not in st.session_state:   # NEW in Lecture 2
    st.session_state['thread_labels'] = get_labels()   # 🔥 load from DB instead of empty {}
...
for thread_id in st.session_state['chat_threads'][::-1]:
    ...
    if cols[0].button(current_label, key=f"open_{thread_id}"):
        ...
        st.rerun()   # 🔥 force sidebar to refresh immediately
...
if user_input:
    ...
    if st.session_state['thread_id'] not in st.session_state['thread_labels']:
        ...
        save_label(st.session_state['thread_id'], label)   # 🔥 persist in DB
